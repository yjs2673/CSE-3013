...
void InitTetris(){
	...
	if(rec_point == 0) DrawShadow(blockY, blockX, nextBlock[0], blockRotate);
...
void DrawChange(char f[HEIGHT][WIDTH],int command,int currentBlock,int blockRotate, int blockY, int blockX){
	...
	//3. 새로운 블록 정보를 그린다. 
	DrawRecommend(recommendY, recommendX, nextBlock[0], recommendR);
	if(rec_point == 0) DrawShadow(blockY, blockX, currentBlock, blockRotate);
		...
		for(int j = 0; j < 4; j++)
		{
			if(block[currentBlock][blockRotate][i][j])
			{
				if((blockY + i >= 0) && (blockY + i < HEIGHT) && (blockX + j >= 0) && (blockX + j < WIDTH))
				{
					...
					if(blockX + j == 9 || blockX + j == 0) wall_count++;
					if(f[blockY + i][blockX + 1 + j] != 0) wall_count++;
					if(f[blockY + i][blockX - 1 + j] != 0) wall_count++;
				}
			}
		}
...
int modified_recommend(RecNode *root)
{
	...
	for(R = 0; R < 4; R++)
	{
		if(nextBlock[root->level] == 0 && R > 1) continue;
		if(nextBlock[root->level] == 4 && R > 0) continue;
		if(nextBlock[root->level] == 5 && R > 1) continue;
		if(nextBlock[root->level] == 6 && R > 1) continue;
		...
		for(X = -2; X < 13; X++)
		{
			...
			root->c[child]->score = root->score + DeleteLine(root->c[child]->f);
            root->c[child]->score += AddBlockToField(root->c[child]->f, nextBlock[root->level], R, --Y, X);
            root->c[child]->score += modified_recommend(root->c[child]);
			root->c[child]->wall += wall_count;

			if(root->c[child]->score > max)
			{
				max = root->c[child]->score;
				prev_wall = root->c[child]->wall;
				if(root->c[child]->level == 1)
				{
					recommendX = X;
					recommendY = Y;
				...
				else if(root->c[child]->score == max)
				{
					max = root->c[child]->score;
					prev_wall = root->c[child]->wall;
					if(root->c[child]->level == 1)
